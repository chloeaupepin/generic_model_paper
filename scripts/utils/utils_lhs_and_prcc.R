...
param_labeller = c("betaC" = bquote("\u03B2"[C]),
                   ...
                   "gammaA_s" = bquote("\u03B3"["A,S"]),
                   "psiA" = bquote("\u03C8"["A"]),
                   "gammaA_r" = bquote("\u03B3"["A,R"]),
                   "Vperc" = bquote(V["perc"]),
                   "vftcs" = bquote(v[a]),
                   "vfds"= bquote(v[d]),
                   "vfis" = bquote(v[i]),
  ...
  epi_prcc_df %>%
    mutate(param_category = case_when(
      ...
      param %in% c("prob_specific_exposure","time_until_decolo_by_specific_ATB","prob_minority_strain_when_infected" ,
                   "prob_specific_exposure_r" ,"time_until_decolo_by_specific_ATB_r", "gammaA_s", "gammaA_r", "psiA") ~ "specific antibiotic exposure",
      param %in% c("Vperc","vftcs", "vfis", "vfds", "vfrs") ~"vaccine"
    ),
    param = factor(param, levels = c("vfrs", "vfis","vfds","vftcs","Vperc",
                                     "gammaA_r", "psiA", "gammaA_s",
